...
[string] $isRgExist = az group exists -n $resourceGroupName
if($isRgExist -eq $false) {
...
az aks create --resource-group=$resourceGroupName --name=$serviceName --dns-name-prefix=$dnsNamePrefix --generate-ssh-keys --node-count=$nodeCount --node-vm-size=$nodeVMSize --vm-set-type $vmSetType --attach-acr $registryName --enable-aad --enable-azure-rbac #--enable-private-cluster --enable-managed-identity --assign-identity <ResourceId> --private-dns-zone system --disable-public-fqdn
...
if ($enableAzureMonitoring) {
    Write-Host "Ensure Resource Provider - Microsoft.OperationsManagement is registered " -ForegroundColor Yellow
