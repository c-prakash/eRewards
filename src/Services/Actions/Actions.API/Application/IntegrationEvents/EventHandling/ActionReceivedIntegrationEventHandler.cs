...
namespace ezLoyalty.Services.Actions.API.Application.IntegrationEvents.EventHandling
{
    ...
    public class ActionReceivedIntegrationEventHandler : IIntegrationEventHandler<ActionReceivedIntegrationEvents>
    {
        ...
        {
            using (LogContext.PushProperty("IntegrationEventContext", $"{@event.Id}-Actions.Domain"))
            {
                _logger.LogInformation("----- Handling integration event: {IntegrationEventId} at {AppName} - ({@IntegrationEvent})", @event.Id, "Actions.Domain", @event);
